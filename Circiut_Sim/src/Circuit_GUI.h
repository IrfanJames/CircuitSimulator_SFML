...
namespace CircuitGUI {
	...
	void renderWinInit() {
		...
		/*sf::RenderWindow app*/// *(;
		app.create(sf::VideoMode(W, H), "CircuitSim", sf::Style::Default, settings);
...
namespace CircuitOptions {
	...
	void deletef() {
		...
		for (int v = 0; v < virSerial.size(); v++)
			comp.erase(comp.begin() + (virSerial[v] - diff++)); //assuming vir or virSerial is sorted(ascendingly)
